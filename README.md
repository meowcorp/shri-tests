...
Для установки зависимостей используй `npm ci`

Тесты **1, 2, 3, 9** расположены на стороне сервера и тестируются с помощью `npm run test:e2e` \
Для прокидывания этих багов нужно каждый раз запускать сервер(команда `npm run start`) прокидывая в аргументы BUG_ID (можно поменять в package.json)

Тесты **4, 5, 6, 7, 8, 10** тестируются с помощью `npm run test:unit` \
Для прокидывания этих багов нужно передать BUG_ID в скрипт запуска юнит тестов. Так же редактируется в package.json

Версию nodejs можно подтянуть с помощью `nvm use` или посмотреть в файле `.nvmrc`

Для связи по проверке домашней работы можете писать мне [@ertome_dev](http://t.me/ertome_dev)
